...
export class DataService {
  ...
  deleteUser(id : Number) : Observable<any>{
    ...
    const deletedUser = this.users.find(it=>it.id === id);
  ...
  deleteRoom(id : Number) : Observable<any>{
    ...
    const deletedRoom = this.rooms.find(it=>it.id === id);
    this.rooms.splice(this.rooms.indexOf(deletedRoom),1);
    return of(null);
  }

  resetUserPassword() : Observable<any>{
    return of(null);
  }
  ...
  getBooking(id : number) : Observable<Booking>{
    const existingBooking = this.bookings.find(it=>it.id === id);
  ...
  updateBooking(booking : Booking) : Observable<Booking>{
    const existingBooking = this.bookings.find(it=>it.id === booking.id);
  ...
  deleteBooking(id : number) : Observable<any>{
    const booingToDelete = this.bookings.find(it=>it.id === id);
    this.bookings.splice(this.bookings.indexOf(booingToDelete),1);
